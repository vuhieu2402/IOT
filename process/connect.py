...
# Biến toàn cục để lưu trữ giá trị trạng thái đèn trước đó
previous_light_status = None
...
def push_message_to_mqtt(value):
    ...
    # So sánh giá trị trạng thái mới với trạng thái trước đó
    if value != previous_light_status:
        print("Push Message to MQTT", value)
        client.publish(topic, value)
        previous_light_status = value

def update_traffic_light(frame, traffic_light_status, countdown_timer, send_to_mqtt):
    red_color = (0, 0, 255)
    green_color = (0, 255, 0)
    ...
    # Gửi message chỉ khi giá trị trạng thái đèn thay đổi và chỉ gửi từ video đầu tiên
    if send_to_mqtt:
...
def create_quad_display(video_paths):
    ...
    yellow_light_duration = 2  # Đặt thời gian đèn vàng là 2 giây
    ...
    # Lấy video đầu tiên
    first_video_capture = video_captures[0]
    ...
    while True:
        ...
        for i, frame in enumerate(frames_resized):
            send_to_mqtt = (i == 0)  # Chỉ gửi giá trị từ video đầu tiên
            if is_yellow_light:
                # Hiển thị đèn vàng nếu đang ở trạng thái đèn vàng
                update_traffic_light(frame, 'Yellow', countdown_timer, send_to_mqtt)
                ...
                update_traffic_light(frame, traffic_light_status, countdown_timer, send_to_mqtt)
